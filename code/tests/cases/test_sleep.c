...
FOSSIL_SUITE(c_sleep_suite);

// Setup function for the test suite
FOSSIL_SETUP(c_sleep_suite) {
    // Setup code here
}

// Teardown function for the test suite
FOSSIL_TEARDOWN(c_sleep_suite) {
    // Teardown code here
}
...
// Test: fossil_time_sleep with various units
FOSSIL_TEST(c_test_sleep_units) {
    struct {
        uint64_t value;
        const char *unit;
    ...
    for (size_t i = 0; i < sizeof(cases)/sizeof(cases[0]); ++i) {
        ...
        fossil_time_sleep(cases[i].value, cases[i].unit);
        uint64_t end = now_ns();
...
// Test: fossil_time_sleep_seconds
FOSSIL_TEST(c_test_sleep_seconds) {
    uint64_t start = now_ns();
    fossil_time_sleep_seconds(1);
    uint64_t end = now_ns();
    assume_sleep_elapsed(start, end, 1000000000ULL);
}

// Test: fossil_time_sleep_milliseconds
FOSSIL_TEST(c_test_sleep_milliseconds) {
    uint64_t start = now_ns();
    fossil_time_sleep_milliseconds(50);
    uint64_t end = now_ns();
    assume_sleep_elapsed(start, end, 50000000ULL);
}

// Test: fossil_time_sleep_microseconds
FOSSIL_TEST(c_test_sleep_microseconds) {
    uint64_t start = now_ns();
    fossil_time_sleep_microseconds(1000);
    uint64_t end = now_ns();
    assume_sleep_elapsed(start, end, 1000000ULL);
}

// Test: fossil_time_sleep_nanoseconds
FOSSIL_TEST(c_test_sleep_nanoseconds) {
    uint64_t start = now_ns();
    fossil_time_sleep_nanoseconds(100000);
    uint64_t end = now_ns();
    assume_sleep_elapsed(start, end, 100000ULL);
}

// Test: fossil_time_sleep_ai with hints
FOSSIL_TEST(c_test_sleep_ai_hints) {
    struct {
        const char *hint;
        uint64_t expected_ns;
    } cases[] = {
    ...
    for (size_t i = 0; i < sizeof(cases)/sizeof(cases[0]); ++i) {
        uint64_t start = now_ns();
        fossil_time_sleep_ai(cases[i].hint);
        uint64_t end = now_ns();
        if (cases[i].expected_ns > 0)
            assume_sleep_elapsed(start, end, cases[i].expected_ns);
        else
...
// Test: fossil_time_sleep with zero and NULL
FOSSIL_TEST(c_test_sleep_zero_and_null) {
    uint64_t start = now_ns();
    fossil_time_sleep(0, "sec");
    fossil_time_sleep(1, NULL);
