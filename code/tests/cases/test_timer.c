...
FOSSIL_SUITE(c_timer_suite);

// Setup function for the test suite
FOSSIL_SETUP(c_timer_suite) {
    // Setup code here
}

// Teardown function for the test suite
FOSSIL_TEARDOWN(c_timer_suite) {
    // Teardown code here
}
...
FOSSIL_TEST(c_test_timer_start_and_elapsed_ns) {
    fossil_time_timer_t timer;
    fossil_time_timer_start(&timer);
    busy_wait_ns(1000000); // 1 ms
    uint64_t elapsed = fossil_time_timer_elapsed_ns(&timer);
    ASSUME_ITS_TRUE(elapsed >= 1000000 && elapsed < 10000000); // 1ms..10ms
}

FOSSIL_TEST(c_test_timer_elapsed_us_and_ms) {
    fossil_time_timer_t timer;
    fossil_time_timer_start(&timer);
    busy_wait_ns(2500000); // 2.5 ms
    uint64_t us = fossil_time_timer_elapsed_us(&timer);
    uint64_t ms = fossil_time_timer_elapsed_ms(&timer);
    ASSUME_ITS_TRUE(us >= 2500 && us < 20000);
    ASSUME_ITS_TRUE(ms >= 2 && ms < 20);
}

FOSSIL_TEST(c_test_timer_elapsed_sec) {
    fossil_time_timer_t timer;
    fossil_time_timer_start(&timer);
    busy_wait_ns(5000000); // 5 ms
    double sec = fossil_time_timer_elapsed_sec(&timer);
    ASSUME_ITS_TRUE(sec >= 0.005 && sec < 0.1);
}

FOSSIL_TEST(c_test_timer_lap_ns) {
    fossil_time_timer_t timer;
    fossil_time_timer_start(&timer);
    busy_wait_ns(1000000); // 1 ms
    uint64_t lap1 = fossil_time_timer_lap_ns(&timer);
    busy_wait_ns(2000000); // 2 ms
    uint64_t lap2 = fossil_time_timer_lap_ns(&timer);
    ASSUME_ITS_TRUE(lap1 >= 1000000 && lap1 < 10000000);
    ASSUME_ITS_TRUE(lap2 >= 2000000 && lap2 < 20000000);
}

FOSSIL_TEST(c_test_timer_null_safety) {
...
FOSSIL_TEST(c_test_timer_hint_ns) {
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("frame"), 16000000ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("tick"), 1000000ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("yield"), 0ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("human_short"), 100000000ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("human_long"), 500000000ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("moment"), 250000000ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns("unknown"), 0ULL);
    ASSUME_ITS_EQUAL_U64(fossil_time_timer_hint_ns(NULL), 0ULL);
}
